...
def create_env(device=None,seed=None, video_kwargs=None, **gym_kwargs):
    ...
    env=gym.make("InvertedDoublePendulum-v4", **gym_kwargs)
    if video_kwargs and video_kwargs["video_folder"]:
    ...
    env=TransformedEnv(env, 
                        Compose(
                            ...
                            RewardScaling(loc=0.0, scale=1/10.0),
                            StepCounter(),
                            )
                        )
    if seed is not None:
...
def get_chkpt_file(run_tag): return os.path.join(base_dir, "chkpts", f"invtdpdlm-{run_tag}.pt")


def train(conf):
    

    tb_logdir=os.path.join(base_dir, "tb_logs","invtdpdlm", run_tag)
    train_state_chkpt_file_best=os.path.join(base_dir, "chkpts", f"invtdpdlm-train-state-{run_tag}.pt")
    train_state_chkpt_file_final=os.path.join(base_dir, "chkpts", f"invtdpdlm-train-state-{run_tag}-final.pt")
    ...
    def eval_train_sync(i_eval_step, n_frame):
        ...
        if eroll["next","step_count"].max().item()==max_eval_steps:
            torch.save(policy.state_dict(), os.path.join(base_dir, "chkpts", f"idpdlm-policy-{run_tag}-peak-final.pt"))
...
def eval_video(conf):
    ...
    env=create_env(device=device, seed=eval_seed, video_kwargs=dict(video_folder=os.path.join(base_dir,"video"), name_prefix=f"invtdpdlm-{eval_run_tag}"))
