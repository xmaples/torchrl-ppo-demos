...
def create_policy_loss(env, *, hidden_dim=128, device=None):
    ...
    adv_module=GAE(gamma=0.99, lmbda=0.99,value_network=value_op, value_key="state_value")
    
    return policy_op, loss_module, adv_module

def create_env(env_id, *, device=None,seed=None, video_kwargs=None, **gym_kwargs):
...
run_tag=f"exp0-{env_id}"
seed=99

def get_chkpt_file(run_tag): return os.path.join(base_dir, "chkpts", f"{run_tag}.pt")
...
def train(conf):
    ...
    tb_logdir=os.path.join(base_dir, "tb_logs", env_id, run_tag)
    train_state_chkpt_file_best=os.path.join(base_dir, "chkpts", f"{env_id}-train-state-{run_tag}.pt")
    train_state_chkpt_file_final=os.path.join(base_dir, "chkpts", f"{env_id}-train-state-{run_tag}-final.pt")
    ...
    def create_serial_env():
        return create_env(env_id, device=device,seed=seed)
    ...
    def eval_train_sync(i_eval_step, n_frame):
        ...
        if eroll["next","step_count"].max().item()==max_eval_steps:
            torch.save(policy.state_dict(), os.path.join(base_dir, "chkpts", f"{env_id}-policy-{run_tag}-peak-final.pt"))
        ...
        score=eroll["next","step_count"].max().item()
        if score>best_score:
            best_score=score
...
def eval_video(conf):
    ...
    env=create_env(env_id,device=device, seed=eval_seed, video_kwargs=dict(video_folder=os.path.join(base_dir,"video"), name_prefix=f"{env_id}-{eval_run_tag}"))
    ...
    policy.eval()
    with set_exploration_mode("mode"):
        td=env.rollout(500, policy)
    logger.info("max step: %s", td["next","step_count"].max().item())
...
def eval_render(conf):
    ...
    eval_run_tag=conf.run_tag or run_tag
    env=create_env(env_id,device=device, seed=seed, render_mode="human")
    policy, *_ = create_policy_loss(env,device=device)
    eval_chkpt_file=conf.chkpt_file or get_chkpt_file(eval_run_tag)
    policy.load_state_dict(torch.load(eval_chkpt_file, map_location=device))
    policy.eval()
    ...
    with set_exploration_mode("mode"):
        td=env.rollout(500, policy)
    last_step=td[-1]
    logger.info("max step: %s", last_step["step_count"].item())
    del env
